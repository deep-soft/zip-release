...
if [[ "$INPUT_TYPE" == "zip" ]] || [[ "$INPUT_TYPE" == "7z" ]]; then
  if [[ "$RUNNER_OS" == "Windows" ]]; then
    ...
    echo "CMD:[7z a -r -ssw -snh -snl -t$INPUT_TYPE $VOL_SIZE $INPUT_FILENAME $INPUT_PATH $INCLUSIONS $EXCLUSIONS $INPUT_CUSTOM]";
    7z a -r -ssw -snh -snl -t$INPUT_TYPE $VOL_SIZE $INPUT_FILENAME $INPUT_PATH $INCLUSIONS $EXCLUSIONS $INPUT_CUSTOM || { printf "\n⛔ Unable to create %s archive.\n" "$INPUT_TYPE"; exit 1;  };
    echo 'Done';
    ...
    if [[ -f $INPUT_FILENAME ]]; then
      ...
      if [[ "$VOLUMES_NUMBER" == "1" ]]; then
        ...
        if [[ "$ARCHIVE_FILENAME" != "" ]]; then
          mv $ARCHIVE_FILENAME $INPUT_FILENAME || true;
          ARCHIVE_FILENAME=$INPUT_FILENAME;
